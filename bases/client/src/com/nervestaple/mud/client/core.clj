(ns com.nervestaple.mud.client.core
  (:require
   ...
   [com.nervestaple.mud.client.mud :as mud])
  (:gen-class))

(def CLI-USAGE
  (->> ["Mud Client"
        ""
        "This application provides the Mud client. If no options or parameters"
        "are provided, the client will be started with the default"
        "configuration."
        ""
        "Options:"]
       (string/join \newline)))
...
(defn cli-handle-arguments
  ...
  [options arguments]
  (let [config (merge (config/read-config "client/config.edn"
    ...
    (mud/create-client config)))

(defn -main
  "Bootstrapping function for the client."
  [& args]
